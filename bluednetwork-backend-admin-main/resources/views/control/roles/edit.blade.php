<x-layouts.authenticated>
    @section('seo')
        <title>Edit Role &raquo; {{ config('app.name') }}</title>
        <meta name="description" content="{{ config('app.name') }} Edit Role">
    @stop
    <x-slot name="header">
    ...
    <div>
        <div class="mx-auto p-3">
            ...
            <form method="post" action="{{ route('control.roles.update', ['role_id' => $role->hashId()]) }}" role="form">
                @method('PUT')
                {!! csrf_field() !!}
                    ...
                    <div class="w-full bg-white rounded-lg overflow-hidden shadow-lg px-6 py-4">
                        ...
                        <div class="flex justify-between mb-6">
                            <div>
                                ...
                                <div class="form-item w-full">
                                    <x-form.label for="guard_type" value="Guard Type" class="font-semibold mb-3">
                                        <x-general.required-field/>
                                    </x-form.label>
                                    <select name="guard_type">
                                        <option value="dashboard" {{ $role->guard_name === 'dashboard' ? 'selected': '' }}>Dashboard (Can only access dashboard)</option>
                                        <option value="frontend" {{ $role->guard_name === 'frontend' ? 'selected' : '' }}>Frontend (Can only access Frontend UI)</option>
                                    </select>
                                    <x-form.input-error for="name" class="mt-2"/>
                                </div>
                            </div>
                                        ...
                                        <div class="form-item w-full m-2">
                                            ...
                                            >
                                                <input 
                                                    ...
                                                    @if (in_array($permission->name, $currentPermissions)) checked @endif
                                                /> 
                                            </x-form.label>
                                        </div>
                                    @endforeach
